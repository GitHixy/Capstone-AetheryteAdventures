      ...
      Object.keys(favsData).forEach(category => {
        if (Array.isArray(favsData[category])) {
          favsData[category].forEach(item => {
            const endpoint = category === 'triadCards' ? 'triad/cards' :
                             category === 'fashionAccessories' ? 'fashions' :
                             category;
...
<div className={styles.itemsContainer}>
            {Object.keys(collectibles).map(section => (
              collectibles[section] && Object.values(collectibles[section]).map(item => (
                <div className={styles.collectibleItem} key={item.id}>
                  <h3 className={styles.collectibleName}>{item.name}</h3>
                  <img className={styles.itemIcon} src={item.icon} alt={item.name}/>
                  <p className={styles.collectibleDescription}>Patch: {item.patch}</p>
                </div>
              ))
            ))}
          </div>
